...
| --- | --- | --- |
| `TELOS_TOOL_MAX_BYTES` | 96 KiB | Max bytes of a single tool's text output (`TELOS_NATIVE_TOOL_MAX_BYTES` remains a legacy alias) |
| `TELOS_TOOL_MAX_LINES` | 400 | Max lines of a single tool's text output (`TELOS_NATIVE_TOOL_MAX_LINES` remains a legacy alias) |
| `TELOS_KEEP_REASONING` | unset | `1` disables stripping of reasoning/COT tags from visible output (`TELOS_NATIVE_KEEP_REASONING` remains a legacy alias) |
