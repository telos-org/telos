...
Local runs require `pi` on `PATH` and model credentials configured for Pi. To
route Pi through a Telos gateway instead, configure one before running:

```bash
telos login
telos configure gateway --mode managed

# or bring your own OpenAI-compatible gateway
telos configure gateway --mode byo --base-url https://gateway.example.com/v1 --api-key ...
```

Pi-backed local runs use the `openai_sync` gateway transport.
