...
listing the files or commands used to check it. Agents are instructed to keep
each update focused on one result in everyday language, including the final
summary. A change that is ready to be checked is distinct from a result confirmed
by a separate check. Updates should describe only the behavior actually checked;
their wording is generated by the model.
