...
func analyzeSessionEvents(session *sessionapi.Session, events []sessionapi.SessionEvent) sessionAnalysis {
	...
	specs := map[string]*sessionSpecAnalysis{}
	if session != nil {
		out.SessionID = session.SessionID
		out.Status = string(session.Status)
		out.Result = ptrString(session.Result)
		out.Completion = ptrString(session.CompletionReason)
		out.CostUSD = ptrFloat(session.TotalCostUSD)
		out.CostUnavailable = ptrBool(session.CostUnavailable)
		out.InputTokens = ptrInt(session.TotalInputTokens)
		out.OutputTokens = ptrInt(session.TotalOutputTokens)
		out.CacheReadTokens = ptrInt(session.TotalCacheReadTokens)
		out.CacheWriteTokens = ptrInt(session.TotalCacheCreateTokens)
		out.Rounds = ptrInt(session.RoundCount)
		for _, spec := range session.Specs {
			name := sessionSpecName(spec)
			specs[name] = &sessionSpecAnalysis{
				Name:            name,
				CostUSD:         ptrFloat(spec.TotalCostUSD),
				CostUnavailable: ptrBool(spec.CostUnavailable),
				InputTokens:     ptrInt(spec.TotalInputTokens),
				OutputTokens:    ptrInt(spec.TotalOutputTokens),
				Rounds:          ptrInt(spec.RoundCount),
				Completion:      ptrString(spec.CompletionReason),
				Failures:        map[string]int{},
			}
		}
	}
	for _, event := range events {
		specName := eventSpecName(event)
		spec := specs[specName]
		if spec == nil && specName != "" {
			spec = &sessionSpecAnalysis{Name: specName, Failures: map[string]int{}}
			specs[specName] = spec
		}
		data := event.Data
		switch event.Event {
		case "game_end":
			result := stringFromAny(data["game_result"])
			completion := stringFromAny(data["completion_reason"])
			errText := stringFromAny(data["error"])
			out.Result = firstNonEmpty(out.Result, result)
			out.Completion = firstNonEmpty(out.Completion, completion)
			if out.Rounds == 0 {
				out.Rounds = intFromAny(data["prover_rounds"]) + intFromAny(data["verifier_rounds"])
			}
			if out.CostUSD == 0 {
				out.CostUSD = floatFromAny(data["total_cost_usd"])
			}
			out.CostUnavailable = out.CostUnavailable || boolFromAny(data["cost_unavailable"])
			if out.InputTokens == 0 {
				out.InputTokens = intFromAny(data["total_input_tokens"])
			}
			if out.OutputTokens == 0 {
				out.OutputTokens = intFromAny(data["total_output_tokens"])
			}
			if spec != nil {
				spec.Result = result
				spec.Completion = completion
				spec.Rounds = intFromAny(data["prover_rounds"]) + intFromAny(data["verifier_rounds"])
				spec.CostUSD = floatFromAny(data["total_cost_usd"])
				spec.CostUnavailable = boolFromAny(data["cost_unavailable"])
				spec.InputTokens = intFromAny(data["total_input_tokens"])
		...
		case "budget_exceeded":
			budget := firstNonEmpty(stringFromAny(data["budget"]), "unknown")
			out.Budgets[budget]++
...
func addFailure(total map[string]int, spec *sessionSpecAnalysis, category string) {
	if category == "" {
		category = "unknown"
	}
	total[category]++
...
func eventSpecName(event sessionapi.SessionEvent) string {
