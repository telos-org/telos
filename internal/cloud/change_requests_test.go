		...
		t.Run(action, func(t *testing.T) {
			...
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				_ = json.NewEncoder(w).Encode(map[string]any{
					...
					"change_request": map[string]any{"id": "req_42", "deployment_id": "sess_123", "action": action, "status": "queued", "review_url": "https://app.example.com/review"},
				})
			}))
			defer srv.Close()
			...
			var err error
			if action == "create" {
				result, err = client.CreateSession(SessionCreateOptions{Name: "books", PackageRef: "@acme/books:1.2.3"})
			} else {
