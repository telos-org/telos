// Package sessionrun implements local Telos session creation and execution.
package sessionrun
...
func RunLocalSessionWithExecutor(sessionDir string, exec game.AgentExecutor) (*game.PVGResult, error) {
	...
	cleanupAgent := func() {
		...
		cleanupDone = true
		if err := agentExec.Cleanup(); err != nil {
			fmt.Fprintf(os.Stderr, "warning: cleanup managed gateway: %v\n", err)
		}
	}
	defer cleanupAgent()
	costHardLimit := agentExec.CostHardLimit()
...
func FinishEpoch(sessionDir string, manifest *sessionapi.Manifest, result *game.PVGResult) error {
	return finishEpoch(sessionDir, manifest, result)
}
...
func LocalWorkerEnv(sessionDir string) []string {
	return localWorkerEnv(sessionDir)
}
